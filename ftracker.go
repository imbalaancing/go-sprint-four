...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    meanSpeedMetInSec := meanSpeed(action, duration) * kmhInMsec
    return ((walkingCaloriesWeightMultiplier * weight + (math.Pow(meanSpeedMetInSec, 2) / (height / cmInM)) * walkingSpeedHeightMultiplier * weight) * duration * minInH)
}
